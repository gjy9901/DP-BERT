    ...
    def forward(self, q, k, v, output_attentions = False):
        ...
        if output_attentions:
            ...
            for head_dim in range(q.shape[1]):
                attn_weights += torch.abs(attn_fn(q[:,head_dim].to(torch.float16), k[:,head_dim].to(torch.float16), torch.eye(v.shape[-2]).to(device).to(torch.float16)))
...
class Gene2VecPositionalEmbedding(nn.Module):
    def __init__(self):
        super().__init__()
        with open("/home/gene2vec_dim_200_iter_9.txt", encoding='utf-8') as f:
            lines = f.readlines()
        gene2vec_weight = []
        for line in lines:
            row = line.split() 
            gene2vec_weight.append(row) 
        gene2vec_weight = np.array(gene2vec_weight) 

        gene2vec_weight = np.delete(gene2vec_weight,0, axis=1)  

        gene2vec_weight = gene2vec_weight.astype(np.float32)
        gene2vec_weight=torch.from_numpy(gene2vec_weight)

        zero_row = torch.zeros(1, gene2vec_weight.shape[1])

        gene2vec_weight = torch.cat((zero_row, gene2vec_weight), dim=0)

        self.emb = nn.Embedding.from_pretrained(gene2vec_weight,padding_idx=0)

    def forward(self, x):
        indices = torch.arange(1, 24448, device=x.device)
        t = torch.where(x == 0, torch.tensor(0, device=x.device), indices)
        return self.emb(t)
...
class Performer(nn.Module):
    def __init__(
        ...
        local_attn_heads = 0,               # num of local attention heads, (heads - local_attn_heads) is num of global performers 局部注意力计算 用不到
...
class MaskLM(nn.Module):
    ...
    def __init__(self, vocab_size, num_hiddens,num_inputs, dropout = 0., **kwargs):
        super(MaskLM, self).__init__(**kwargs)
        self.mlp = nn.Sequential(nn.Linear(num_inputs, num_hiddens),
                                 nn.ReLU(),
                                 nn.Dropout(dropout),
                                 nn.Linear(num_hiddens, 4096),
                                 nn.ReLU(),
                                 nn.Dropout(dropout),
...
class PerformerLM(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.token_emb = nn.Embedding(num_tokens, dim,padding_idx=0)
        ...
        self.mlm = MaskLM(vocab_size, num_hiddens, dim)
    ...
    def forward(self, x,pred_positions,return_encodings = True, output_attentions = False, **kwargs):
        ...
        if output_attentions:
            ...
            # norm and to logits
            x = self.norm(x)
            mlm_Y_hat = self.mlm(x, pred_positions)
            
            if return_encodings:
                return x,mlm_Y_hat, attn_weights
